...
func (c *Chunk64MXLIST32) Identifier() Identifier64 {
	return Identifier64{'M', 'X', 'L', 'I', 'S', 'T', '3', '2'}
}

// Returns the total length of the chunk, including headers and such.
func (c *Chunk64MXLIST32) Length() int64 {
...
func (c *Chunk64MXLIST32) Chunks() iter.Seq2[Chunk32, error] {
	return func(yield func(Chunk32, error) bool) {
		...
		for chunkPos < c.dataStartOffset+int64(c.Header.DataLength) {
			...
			sc, err := c.Accessor.ReadChunk32()
			if err != nil {
				yield(nil, fmt.Errorf("failed to read chunk: %w", err))
				return
			}

			if chunkPos+int64(sc.Length()) > c.dataStartOffset+int64(c.Header.DataLength) {
...
func (*Chunk64MXLIST32) BuildChunk(a *Accessor) (Chunk64, error) {
	if a == nil {
		return nil, fmt.Errorf("accessor is nil")
	}

	c := &Chunk64MXLIST32{Accessor: a}
...
func init() {
	MustRegisterChunk64(&Chunk64MXLIST32{})
}
